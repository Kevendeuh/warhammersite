...
function renderCart() {
    ...
    // Remove any existing item rows (keep empty state element)
    ...
    cart.forEach(item => {
      ...
      <div>
        ...
        <div class="cart-item-qty">
          <button class="qty-btn" data-action="dec" data-id="${item.id}" aria-label="Diminuer la quantité" type="button">−</button>
          <span aria-label="Quantité: ${item.quantite}">${item.quantite}</span>
          <button class="qty-btn" data-action="inc" data-id="${item.id}" aria-label="Augmenter la quantité" type="button">+</button>
        </div>
        <button class="remove-btn" data-id="${item.id}" type="button" aria-label="Retirer ${item.nom} du Drop Pod">
          ✕ Retirer
        </button>
      </div>
...
function handleCartAction(e) {
    ...
    if (incBtn) {
        ...
        else if (item && item.quantite === 1) cart = cart.filter(i => i.id !== id);
    } else if (remBtn) {
...
async function handleSubmit() {
    ...
    try {
        ...
        if (!res.ok) {
            // If auth required but not logged in — still simulate locally
            if (res.status === 401) {
                const fakeCode = 'CMD-' + Date.now();
                launchOrbitalStrike(fakeCode);
                return;
            }
        ...
        // On any error, still run the animation in demo mode
        const fakeCode = 'CMD-DEMO-' + Math.floor(Math.random() * 99999);
        launchOrbitalStrike(fakeCode);
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const strikeBtn = document.getElementById('strike-btn');
    if (strikeBtn) {
        strikeBtn.addEventListener('click', handleSubmit);
    }
});
